...
export function CitationPanel() {
  ...
  return (
    <aside className="hidden min-h-0 flex-col gap-4 border-l border-white/10 bg-black/20 p-4 xl:flex">
      <DocumentsPanel />
      <section className="glass min-h-0 flex-1 rounded-xl p-4">
        <div className="mb-3 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-white">Citations</h2>
            <p className="text-xs text-slate-500">{citations.length} retrieved references</p>
          </div>
          <ExternalLink className="h-4 w-4 text-slate-500" />
        </div>
        <div className="grid max-h-[calc(100vh-25rem)] gap-2 overflow-y-auto pr-1">
          {citations.map((citation, index) => (
            ...
            >
              ...
              <div className="mt-1 line-clamp-2 text-sm text-slate-100">{citationTitle(citation)}</div>
              <p className="mt-2 line-clamp-3 text-xs leading-5 text-slate-500">{citationSnippet(citation)}</p>
            </button>
          ))}
          {!citations.length && (
